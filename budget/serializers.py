...
class BudgetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Budget
        fields = ['id',  'amount', 'month_year', 'user', 'category',
                  'notification_threshold','spent_amount' ,
                  'year', 'month', 'is_deleted']
        read_only_fields = ['id','year', 'month', 'notfiication_threshold','is_deleted','spent_amount']
    ...
    def validate_category(self, value):
        ...
        if not user.is_staff:
            ...
            if not (value.is_predefined or value.user_id == user.id):
                raise ValidationError("You can only create budgets for your categories or predefined categories")
    ...
    def validate_user(self, value):
        ...
        if current_user.is_staff:
            if target_user_id == current_user.id:
                raise ValidationError("Staff members cannot create budgets for themselves")
            
            target_user = CustomUser.objects.filter(id=target_user_id, is_active=True).first()
            if not target_user:
